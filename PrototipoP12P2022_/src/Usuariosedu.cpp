...
void mostrarLineaUs( ostream&, const datosUsuarioEdu & );
...
void imprimirRegistroUs( fstream &leerDeArchivo )
{
    ...
    datosUsuarioEdu usuario;
    leerDeArchivo.read( reinterpret_cast< char * >( &usuario ), sizeof( datosUsuarioEdu ) );
...
void mostrarLineaUs( ostream &salida, const datosUsuarioEdu &registro )
{
...
void crearArchivoCreditoUs()
{
    ...
    datosUsuarioEdu usuarioEnBlanco;
    for ( int i = 0; i < 100; i++ )
        creditoSalida.write(reinterpret_cast< const char * >( &usuarioEnBlanco ), sizeof( datosUsuarioEdu ) );
}
void nuevoRegistroUs( fstream &insertarEnArchivo )
{
    int usuario = obtenernUsuario( "\nEscriba el Usuario " );
    insertarEnArchivo.seekg( ( usuario - 1 ) * sizeof( datosUsuarioEdu ) );
    datosUsuarioEdu usuarioos;
    insertarEnArchivo.read( reinterpret_cast< char * >( &usuarioos ), sizeof( datosUsuarioEdu ) );
    ...
    if ( usuarioos.obtenerUsuario() == 0 ) {
        ...
        insertarEnArchivo.seekp( ( usuario - 1 ) * sizeof( datosUsuarioEdu ) );
        insertarEnArchivo.write( reinterpret_cast< const char * >( &usuarioos ), sizeof( datosUsuarioEdu ) );
...
void modificarRegistroUs( fstream &verificarArchivoUsuario )
{
    ...
    do{
        cout<<"\t\t\t---------------------------------------------------------------------------------------"<<endl;
        cout<<"\t\t\t |   INICIO DE SESION - SISTEMA EDUCATIVO 9959-21-1395 DANIEL ALEXANDER HALL ALVAREZ  |"<<endl;
        cout<<"\t\t\t---------------------------------------------------------------------------------------"<<endl;
        ...
        verificarArchivoUsuario.seekg(( usuario - 1 ) * sizeof( datosUsuarioEdu ));
        verificarArchivoUsuario.read( reinterpret_cast< char * >( &usuarioos ), sizeof( datosUsuarioEdu ) );
